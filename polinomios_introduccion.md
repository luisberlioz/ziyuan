...
### 5.7 Binomio a la $n$ y el triángulo de Pascal

#### Idea general

Al desarrollar $(a + b)^n$ para $n = 0, 1, 2, 3, \dots$ se obtienen polinomios cuyos **coeficientes** siguen un patrón muy regular, conocido como el **triángulo de Pascal**. Saberlo permite escribir el desarrollo de $(a + b)^n$ sin hacer multiplicaciones término a término.

#### El triángulo

Cada fila $n$ da los coeficientes de $(a + b)^n$:

$$
\begin{array}{c|l}
n & \text{Coeficientes de } (a+b)^n \\ \hline
0 & 1 \\
1 & 1 \quad 1 \\
2 & 1 \quad 2 \quad 1 \\
...
- **Bordes:** cada fila empieza y termina en $1$.
- **Interior:** cada número es la **suma de los dos números que tiene justo encima** en la fila anterior.

Por ejemplo, en la fila $5$ el $10$ central se obtiene como $4 + 6$ (los dos números encima) y el otro $10$ como $6 + 4$.

#### Propiedad clave: elBinomio de Newton

Para cualquier entero $n \geq 0$:

$$(a + b)^n = \sum_{k=0}^{n} \binom{n}{k} a^{n-k} b^{k}$$

donde los **coeficientes binomiales** $\binom{n}{k}$ ("$n$ sobre $k$") se leen directamente de la fila $n$ del triángulo y satisfacen:

$$\binom{n}{k} = \binom{n-1}{k-1} + \binom{n-1}{k} \quad \text{(regla de Pascal)}$$

- $\binom{n}{0} = \binom{n}{n} = 1$ (los bordes del triángulo).
- $\binom{n}{k}$ es el coeficiente del término $a^{n-k} b^{k}$.

#### Ejemplo: desarrollar $(2x + 3)^4$

1. La fila $4$ del triángulo es $1, 4, 6, 4, 1$.
2. Los exponentes de $a = 2x$ bajan de $4$ a $0$; los de $b = 3$ suben de $0$ a $4$:

$$(2x + 3)^4 = 1\cdot(2x)^4 + 4\cdot(2x)^3(3) + 6\cdot(2x)^2(3)^2 + 4\cdot(2x)(3)^3 + 1\cdot(3)^4$$

3. Se calcula cada término:
...
#### Resumen de los productos notables como casos particulares

| Exponente $n$ | Desarrollo |
|---|---|
| $n = 0$ | $(a+b)^0 = 1$ |
| $n = 1$ | $(a+b)^1 = a + b$ |
...
Los casos $n=2$ y $n=3$ son los que se llaman usualmente **productos notables**; $n \geq 4$ se obtiene con la misma regla y los coeficientes del triángulo de Pascal.
